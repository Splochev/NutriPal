...
import { getFirestore, collection, doc, getDocs, getDoc, deleteDoc, setDoc, updateDoc } from 'firebase/firestore';
...
function useFoodFirestore() {
    ...
    const getAllFoods = async () => {
        ...
        try {
            ...
            (await getDocs(collection(db, 'nutripal')))
                .forEach((doc) => {
    ...
    const searchFoodNames = (keyword: string) => {
        if ($foodNames.value.length) {
            ...
            const filteredFoodNames = foodNames.filter(foodName => foodName.includes(keyword));
    ...
    const addFood = async (food: FoodDto) => {
        try {
            ...
            $foodNames.set([...$foodNames.value.filter(name => name !== food.name)]);
    ...
    const deleteFood = async (foodName: string) => {
        try {
            ...
            await resetFoodNames($foodNames.value.filter(name => name !== foodName));
        } catch (error) {
            console.error('Error deleting document:', error);
        }
    };

    const arePropertiesChanged = (
        oldFood: FoodDto,
        newFood: FoodDto
    ) => {
        return (
    ...
    const editFood = async (foodName: string, updatedFood: FoodDto) => {
        try {
            ...
            //@ts-ignore
            const arePropsChanged = arePropertiesChanged(JSON.parse(oldDocumentData.data) as FoodDto, updatedFood);

            if (isFoodNameChanged) {
                const newDocumentRef = doc(collectionRef, encodeURIComponent(updatedFood.name));
            ...
            if (isFoodNameChanged) {
                await resetFoodNames($foodNames.value.map(name => (name === foodName ? updatedFood.name : name)));
            }
        } catch (error) {
    ...
    return { $foodList, $loading, $foodNames, deleteFood, addFood, editFood, searchFoodNames, getFood, getAllFoods };
}
