...
export const calculateNutritionValue = (
  ...
  if (grams) {
    return (
      Math.round(
        (((value || 0) * Number(grams)) / 100 + Number.EPSILON) * 100
      ) / 100
    );
  }
  return "";
...
export const deleteFoodFromChosenFoods = (
  ...
  const filteredValues = $chosenFoods.value.filter(
    (food) =>
      (typeof food === "object" && food.name !== foodName) ||
      (typeof food === "string" && food !== foodName)
  );
...
export const showButtonInInterval = (
  ...
  if (
    ...
    ($draggedStartTime.value || $draggedStartTime.value === 0)
  ) {
  ...
  if ($itemForDeletion.value) {
    setTimeout(() => {
      if (typeof $itemForDeletion.value === "object") {
        const leftChosenFoods = deleteFoodFromChosenFoods(
          $itemForDeletion.value.name,
          $chosenFoodsGrams,
          $chosenFoods,
          $foodList
        );
        const hasFoodDto = leftChosenFoods.find(
        ...
        $chosenFoods.set(
          $chosenFoods.value.filter((item) => item !== $itemForDeletion.value)
        );
      }
      $itemForDeletion.set("");
    }, 500);
