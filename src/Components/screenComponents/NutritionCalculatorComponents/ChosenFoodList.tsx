...
import { useNutritionCalculatorStyles, deleteFood, startDraggingListItem, showButtonInInterval, checkIfCursorIsOnDeleteButton, deleteFoodIfDroppedOnDeleteButton } from '../../../Services/NutritionCalculatorService';
...
export const ChosenFoodList: React.FC<ChosenFoodListProps> = ({ $chosenFoods, $chosenFoodsGrams, $foodList }) => {
    ...
    const onMouseUpDeleteListItem = (event: any) => {
        deleteFoodIfDroppedOnDeleteButton(event, $draggedListItem, $showButton, $draggedStartTime, $itemForDeletion, onMouseUpDeleteListItem);
    };
    ...
    useEffect(() => {
        ...
        if (deleteButtonElement) document.addEventListener("mousemove", onMouseMoveOverDeleteButton);
        return () => document.removeEventListener("mousemove", onMouseMoveOverDeleteButton);
    }, []);

    useEffect(() => {
        if ($draggedListItem.value) document.addEventListener('mouseup', onMouseUpDeleteListItem);
        else document.removeEventListener('mouseup', onMouseUpDeleteListItem);
        return () => document.removeEventListener('mouseup', onMouseUpDeleteListItem);
    ...
    return (
        <div>
            <ReactMovableList
                ...
                onChange={({ oldIndex, newIndex }) => $chosenFoods.set(arrayMove($chosenFoods.value, oldIndex, newIndex))}
                renderList={({ children, props }) => <List {...props}>{children}</List>}
                renderItem={({ value, props, isDragged }) => {
                    startDraggingListItem(isDragged, value, $showButton, $draggedStartTime, $draggedListItem);
                    return (
                        typeof value === 'object' ?
                            <ListItem
                                {...props}
                                ...
                                className={clsx(
                                    classes.listItem,
                                    $showButton.value && $draggedListItem.value === value ? classes.draggedListItem : null
                                )}
                            >
                                <ListItemContent
                                    foodDto={value}
            ...
            <div
                className={clsx(
                    ...
                    $showButton.value ? classes.showDeleteListItemBtn : classes.hideDeleteListItemBtn
                )}
            >
                <CoreIconButton
