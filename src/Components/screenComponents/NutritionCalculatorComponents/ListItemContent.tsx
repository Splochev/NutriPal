      ...
      <div className={classes.nutritionContainer}>
        <div className={classes.label}>
          ...
          <CoreIconButton
            ...
            icon={<RestartAltIcon />}
            onClick={() =>
              resetChosenFoodsGrams($chosenFoodsGrams, foodDto.name)
            }
          />
        </div>
        <TextField
          ...
          onChange={(e) =>
            setChosenFoodsGrams(e, foodDto.name, $chosenFoodsGrams)
          }
          type="number"
          variant="outlined"
          InputProps={{
        ...
        {MACRO_NUTRIENTS.map((nutrient) => (
          <div className={classes.styledNutrition} key={nutrient.name}>
            ...
            <Typography variant="subtitle1" className={classes.nutritionValue}>
              {calculateNutritionValue(
                // @ts-ignore
                foodDto[nutrient.name],
                foodDto.name,
                $chosenFoodsGrams
              )}
            </Typography>
            <Typography variant="subtitle1">{nutrient.alias}</Typography>
          </div>
        ))}
      </div>
    </>
